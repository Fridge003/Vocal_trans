...
class TestDataset(object):
    ...
    def __init__(self, config):
        assert config.trg_spk in speakers, f'The trg_spk should be chosen from {speakers}, but you choose {self.trg_spk}.'
        # Source speaker
...
class config_api:
    ...
    def __init__(self, file_path, trg_spk, save_path, src_spk='p001', resume_iters=20000, num_converted_wavs=4):
        ...
        self.num_converted_wavs = num_converted_wavs
...
def test_api(file_path, trg_spk, save_path):
    api = config_api(file_path, trg_spk, save_path)
    test(api)
